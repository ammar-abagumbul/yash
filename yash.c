...
void
free_tokens(char **tokens, int num_commands) {
...
void
tokenize(Command *cmd, char *buffer)
{
    if (cmd == NULL || buffer == NULL || *buffer == '\0' || *buffer == '|' || *buffer == '\n') {
        perror("[ERROR] tokenization failed. Make sure to provide valid cmd and buffer");
        return;
    }
...
int
readline(char *buffer, size_t size)
{
...
main(int argc, char *argv[])
{
    ...
    do {
        printf("%s\n", PREFIX);
        if (readline(buffer, sizeof(buffer)) == 1) // Do nothing
        {
            if (!validate_cmd(buffer)) continue;
            Command cmd;
            tokenize(&cmd, buffer);
